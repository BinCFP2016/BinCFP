 ...
 */
/*
Jiang modified it to filter out share lib reps
*/
#include "pin.H"
#include <iostream>
#include <fstream>
...
static KNOB<string> KnobOutput(KNOB_MODE_WRITEONCE, "pintool", "o", "countreps-max.out", "output file");
...
class stats
{
    ...
    UINT64 maxCount;                            /* max number of rep iteration, added by jiang ming */
public:
    stats() : count(0), repeatedCount(0), zeroLength(0),maxCount(0) {}
    VOID output() const;
...
VOID stats::output() const
{
    ...
    out << std::setw(fieldWidth) << maxCount;
    out << endl;
}
...
static VOID printDynamicIntructionCounts()
{
    ...
    out << "Op              Count  Repeated Count       %      Zero Count   Mean reps(NZ)   MaxRepCount" << endl;
